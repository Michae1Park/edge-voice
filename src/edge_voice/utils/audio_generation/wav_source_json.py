...
Run as a separate process:
    python -m edge_voice.utils.audio_generation.wav_source_json --wav file.wav --channels rx tx
"""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if file_sr != args.sr:
        logger.info("WavSourceJSON: resampling %d → %d Hz", file_sr, args.sr)
        raw_data = _resample(raw_data, file_sr, args.sr)

    total_chunks = math.ceil(len(raw_data) / args.chunk)
    logger.info(
        "WavSourceJSON: %s %d Hz %d samples → %d frames × %d channel(s) → %s",
        args.wav,
        file_sr,
        len(raw_data),
        total_chunks,
    ...
    logger.info(
        "WavSourceJSON: done — %d frames in %.2fs (%.1fx real-time)",
