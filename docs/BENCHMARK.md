...
- **Overlap higher, speedup lower than the dev box's — a real pattern, not yet a confirmed cause.** On the RPi5 the two processes were computing simultaneously effectively the entire time (100%), more cleanly than the dev box's 84%. Despite that cleaner overlap, speedup is lower (1.47x vs 1.70x). Memory bandwidth or shared cache contention is the leading hypothesis for why — some shared hardware resource below the CPU core level capping the payoff — but this was never independently measured (no bandwidth/cache-miss profiling run here), so it's a plausible explanation, not a confirmed one. What is confirmed: the GIL-removal mechanism holds, even more cleanly, on target hardware; the payoff is real but smaller than the dev box's number suggested — **1.47x is what to plan around**, not the dev box's more optimistic 1.70x.
